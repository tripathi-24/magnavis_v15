#!/usr/bin/env python3
"""
Build a performance matrix figure (heatmap + numeric table) from ``comparison_table.csv``
produced by ``run_suite.py``.

Writes into the chosen run directory (default: latest under ``results/``):
  - ``perform_matrix_heatmap.png`` — metrics heatmap (0–1 scores)
  - ``perform_matrix_table.png`` — numeric counts + metrics table (Model column left-aligned, moderate width)
  - ``perform_matrix.csv`` (same rows, human-readable model labels)

Usage::

  cd src/benchmark_feb13_2026
  python generate_perform_matrix.py
  python generate_perform_matrix.py --results-dir results/20260422_104826
"""
...
# Row label in tables / plots (internal CSV ``predictor`` key -> display name)
DISPLAY_NAMES: Dict[str, str] = {
    ...
    "transformer_pretrained": "Pretrained Keras forecaster",
    "gru_fresh": "GRU (fresh)",
...
def _model_column_width_fraction(labels: List[str], n_cols: int) -> List[float]:
    """First column (Model): half the relative width of the previous formula; rest split evenly."""
...
def _plot_numeric_table(rows: List[Dict[str, Any]], out_png: Path, title: str) -> None:
    ...
    max_len = max((len(s) for s in labels), default=12)
    # Figure width: less extra slack for long model names (matches narrower Model col).
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Generate perform_matrix_heatmap.png + perform_matrix_table.png from comparison_table.csv"
    )
    ap.add_argument(
        "--results-dir",
        type=Path,
        default=None,
        help="Run folder containing comparison_table.csv (default: latest under results/)",
    )
    args = ap.parse_args()
    ...
    title = f"Feb 13 benchmark — {run_dir.name}"
